...
{	
            ...
            if (!cfg.maxSelection || _selection.length < cfg.maxSelection) {
				...
				$.each(items, function(index, json) {
                    if ($.inArray(json[cfg.valueField], ms.getValue()) === -1) {
						var found = false;
						for (var i = 0; i < interests.length && !found; i++) {
						  if (interests[i] === json.name) {
							found = true;
							break;
						  }
						}
	...
	var jsonData = [];
		...
		var addInterest = function(fruitJson) {
			var interestToBeadded = fruitJson.name;
			interestToBeadded = interestToBeadded.replace(/\s+/g, '_').toUpperCase();
			
			var addInterestJsonData = "{\"userId\":" + sessionStorage.getItem("userId") + ", \"interestEnum\":\"" + interestToBeadded + "\"}";
            $.ajax({
                type: "POST",
				method: "POST",
                url: "https://904xhviqh5.execute-api.us-east-2.amazonaws.com/MyBuddyPOC/interest",
				data: addInterestJsonData,
		...
		var getInterests = function() {
            $.ajax({
                ...
                success: function(msg){
                    if (msg.messageFromServer == "Success") {
						var getUserJsonInterests = msg.interests;
						for(var i=0; i<getUserJsonInterests.length; i++) {
							for (var j=0; j<jsonData.length; j++) {
								if (jsonData[j].name.toLowerCase() === getUserJsonInterests[i].interestEnum.replace('_', ' ').toLowerCase()) {
									var item = {id:j, name:interests[j], interestId:getUserJsonInterests[i].interestId};
					...
					var ts = new TagSuggest();
					ts.addToSelection(userInterests, true);
                },
				 error: function (jqXHR, textStatus, errorThrown) {
					console.log(jqXHR.responseText)
				}, 
        ...
        $(document).ready(function() {
            for(var i=0;i<interests.length;i++) jsonData.push({id:i,name:interests[i]});
